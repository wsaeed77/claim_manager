...
export default function PayoutModal({ show, onClose, client, payout = null, partners = [] }) {
    ...
    useEffect(() => {
        if (payout) {
            // Editing existing payout
            setData({
                client_id: payout.client_id || client?.id || '',
                inv_no: payout.inv_no || '',
                status: payout.status || '',
                indate: payout.indate ? (typeof payout.indate === 'string' && payout.indate.includes('-') ? payout.indate : new Date(payout.indate).toISOString().split('T')[0]) : '',
                desc_de: payout.desc_de || '',
                uto: payout.uto || '',
                uto_type: payout.uto_type || '',
                ufor: payout.ufor || '',
                amount: payout.amount || '',
                vat: payout.vat || '',
                total: payout.total || '',
                date_paid: payout.date_paid ? (typeof payout.date_paid === 'string' && payout.date_paid.includes('-') ? payout.date_paid.split('-').reverse().join('-') : new Date(payout.date_paid).toISOString().split('T')[0]) : '',
                clawed_date: payout.clawed_date ? (typeof payout.clawed_date === 'string' && payout.clawed_date.includes('-') ? payout.clawed_date.split('-').reverse().join('-') : new Date(payout.clawed_date).toISOString().split('T')[0]) : '',
                chq_no: payout.chq_no || '',
                due_date: payout.due_date ? (typeof payout.due_date === 'string' && payout.due_date.includes('-') ? payout.due_date.split('-').reverse().join('-') : new Date(payout.due_date).toISOString().split('T')[0]) : '',
                type: 'payout',
            });
        } else {
            // Creating new payout
    ...
    const handleSubmit = (e) => {
        ...
        if (payout) {
            ...
            const updateData = { ...submitData, id: payout.id };
            router.post('/store_update_payout', updateData, {
                preserveScroll: true,
                onSuccess: () => {
                ...
                onError: () => {
                    toast.error('Failed to update payout. Please check the form for errors.');
                },
            });
        } else {
            // Create new payout
            post('/insert_payout', {
                ...
                onError: () => {
                    toast.error('Failed to create payout. Please check the form for errors.');
                },
            });
        }
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                    ...
                    <PrimaryButton disabled={processing} className="px-6 py-2.5">
                        {processing ? 'Saving...' : payout ? 'UPDATE PAYOUT' : 'CREATE PAYOUT'}
                    </PrimaryButton>
                </div>
            </form>
        </Modal>
